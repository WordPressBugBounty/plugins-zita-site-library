 ...
 * @since  1.0.0
 */
class Zita_Site_Library_Options_Import {
		...
		if ( isset( $cats ) ) {
			...
			foreach ( $cats as $key => $cat ) {
				...
				if ( ! empty( $cat['slug'] ) && ! empty( $cat['thumbnail_src'] ) ) {
					...
					$image = (object) Zita_Site_Library_Helper::_sideload_image( $cat['thumbnail_src'] );
	...
	private function insert_logo( $image_url = '' ) {
		...
		$data = (object) Zita_Site_Library_Helper::_sideload_image( $image_url );
